      ...
      {!isLoading && review && (
        <Card>
          <CardHeader className="p-4">
            <div className="flex gap-3 items-center">
              ...
              <div>
                ...
                <p className="text-xs text-muted-foreground">評價日期：{formatDate(review.created_at)}</p>
              </div>
            </div>
          </CardHeader>
          ...
          <CardContent className="flex flex-col gap-3">
            ...
            {review.review === null ? <SectionFallback icon={MessageCircle} iconSize={20} textSize="text-sm" fallbackText={"未有留言"} /> : <p className="text-sm">{review.review}</p>}
            {review.photos?.length > 0 && (
              <div className="grid gap-2 grid-cols-3 sm:grid-cols-5 md:grid-cols-6 lg:grid-cols-10">
                <ReviewImages imageList={review.photos} userProfile={{ icon: review.user_icon, name: review.username }} />
              </div>
            )}
          </CardContent>
        </Card>
