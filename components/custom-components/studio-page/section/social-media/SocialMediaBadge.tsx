...
const SocialMediaBadge = ({ type, value }: Props) => {
    ...
    <a href={value} target="_blank" rel="noopener noreferrer">
      <div className="flex items-center gap-2 group">
        <div className="rounded-full bg-brand-50 p-3 w-fit group-hover:bg-brand-100 transition duration-300">
          <Image src={socialMedia.icon} alt={`${socialMedia.label} Icon`} width={20} height={20} />
        </div>
        <p className="group-hover:text-primary transition duration-300">{socialMedia.label}</p>
      </div>
    </a>
  );
};
