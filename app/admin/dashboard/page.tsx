    ...
    <div>
      ...
      <div className="grid grid-cols-3 md:grid-cols-5 gap-4 mb-5">
        <ScoreCard
          ...
          toolTipContent={<p>Website registration total amount</p>}
          isLoading={isLoading}
        />
        <ScoreCard
          metricName={"Bookings"}
          value={data?.bookingCount?.total}
          toolTipContent={<p>Booking total amount</p>}
          isLoading={isLoading}
        />
        <ScoreCard
          metricName={"Studios"}
          value={data?.activeStudioCount?.total}
          toolTipContent={<p>Active studio total amount</p>}
          isLoading={isLoading}
        />
        <ScoreCard
          metricName={"Booking Revenue"}
          value={data?.bookingRevenue?.total}
          toolTipContent={<p>Booking revenue total amount</p>}
          isLoading={isLoading}
          isRevenue={true}
        />
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-8">
        <Card className="border-gray-200">
          ...
          <CardContent className="pt-2 md:pt-1 pb-3 px-3 space-y-5">
            <ByMonthBarChart
              chartData={data?.userCount?.monthBreakdown}
              label={"Users"}
              isLoading={isLoading}
            />
          </CardContent>
        ...
        <Card className="border-gray-200 h-fit">
          ...
          <CardContent className="pt-2 md:pt-1 px-3 space-y-5">
            ...
            {data?.top5BookingStudio.length === 0 && (
              <SectionFallback icon={House} fallbackText={"此時間段沒有預約"} textSize="text-sm" />
            )}
          </CardContent>
        </Card>
      </div>
