...
const BookmarksPage = async (props: Props) => {
  ...
  if (!user) {
    return <ToastMessageWithRedirect type={"error"} message={"請先登入才可操作"} redirectPath={"/auth/login"} />;
  }
  const searchParams = await props.searchParams;
  const currentPage = Number(searchParams["page"]) || 1;
  const bookmarkResult = await userService.getUserAllStudioBookmark(user?.id, currentPage, pageSize);
  const bookmarkList = bookmarkResult.success ? bookmarkResult.data : { bookmarkList: [], totalCount: 0 };
  ...
  return (
    <div>
      ...
      <div className="mt-8">
        {Number(bookmarkList?.totalCount) > pageSize && <PaginationWrapper currentPage={currentPage} itemCount={Number(bookmarkList?.totalCount) || 0} pageSize={pageSize} useQueryString={true} />}
      </div>
    </div>
  );
};
