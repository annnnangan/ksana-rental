...
const ContactPage = async ({ params }: { params: Promise<{ id: string }> }) => {
  ...
  const isCompletedAllStepsResponse = await studioService.checkIfCompletedAllOnboardingSteps(
    studioId
  );
  ...
  return (
    <>
      <StepIntro
        title={"確認申請"}
        description="請閱讀和同意條款與細則，並送出你的申請，Ksana會於3-7個工作天內審查你的申請。"
      ></StepIntro>
      <ConfirmationForm
        studioId={studioId}
        isFilledAllSteps={isCompletedAllStepsResponse.success}
      />
