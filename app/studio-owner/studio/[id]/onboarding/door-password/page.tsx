...
const DoorPasswordPage = async ({ params }: { params: Promise<{ id: string }> }) => {
  ...
  const defaultValueResult = await studioService.getDoorPassword(studioId, userId);
  ...
  return (
    <>
      <div>
        ...
        <p className="text-sm md:text-base mb-6">Ksana會於預約2小時前於平台上自動發送場地大門密碼給場地租用用戶。請選擇是否同意Ksana發送場地大門密碼給場地租用用戶。</p>
      </div>
