...
import { allowedImageMineTypes, formattedMineTypes, maxReviewImageCount, maxReviewImageSize } from "../file";
...
export const reviewBookingSchema = z.object({
  ...
  review: z.string().min(10, "請以不少於10字寫下你的評論。").max(200, "評論最多只可填寫200字。"),
  is_anonymous: z.boolean(),
  is_complaint: z.boolean(),
  images: z.array(singleImageSchema).max(maxReviewImageCount, `最多只可上傳${maxReviewImageCount}張圖片。`),
});
